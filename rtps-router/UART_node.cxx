...
int UART_node::parseRTPSfromUART(char* topic_ID, uint8_t *seq, char buffer[], size_t buflen)
{
    ...
    if (_buf_size < RTPS_HEADER_SIZE) // starting ">>>" + topic + seq + len + crchi + crclow
        return 0;

    // look for starting ">>>"
    while (start <= (_buf_size - RTPS_HEADER_SIZE) && memcmp(_buf + start, ">>>", 3) != 0) {
        start++;
    }
    if (start >= (_buf_size - RTPS_HEADER_SIZE)) {
        return 0;
    }
    ...
    *seq = _buf[start + 4];
    memmove(buffer, _buf + start + RTPS_HEADER_SIZE, len);

    memmove(_buf + start, _buf + start + RTPS_HEADER_SIZE + len, sizeof(_buf) - start - RTPS_HEADER_SIZE - len);
    _buf_size -= len + RTPS_HEADER_SIZE;
...
int UART_node::writeRTPStoUART(const char topic_ID, char buffer[], uint32_t length)
{
    ...
    static char pre[] = ">>>TSLcc"; // ugly hack in place
    uint8_t len = length;
    ...
    pre[6] = (crc >> 8) & 0xff;
    pre[7] = crc & 0xff;
    int r = write(m_uart_filestream, pre, sizeof(pre) - 1);
    int s = write(m_uart_filestream, buffer, length);
    printf("%d %d\n", r, s);
